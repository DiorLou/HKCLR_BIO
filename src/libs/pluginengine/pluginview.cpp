...
/*!
    ...
    \sa PluginEngine::PluginDetailsView
    \sa PluginEngine::PluginErrorView
*/

/*!
    \fn void PluginView::currentPluginChanged(PluginEngine::PluginSpec *spec)
    The current selection in the plugin list has changed to the
    plugin corresponding to \a spec.
*/

/*!
    \fn void PluginView::pluginActivated(PluginEngine::PluginSpec *spec)
    The plugin list entry corresponding to \a spec has been activated,
    for example by a double-click.
*/

Q_DECLARE_METATYPE(PluginEngine::PluginSpec *)
Q_DECLARE_METATYPE(PluginEngine::PluginCollection *)
...
static const QIcon &icon(IconIndex icon)
{
    ...
    case OkIcon: {
        static const QIcon ok
            = QIcon(":/pluginengine/images/ok.png");
        return ok;
    }
    case ErrorIcon: {
        static const QIcon error
            = QIcon(":/pluginengine/images/error.png");
        return error;
    }
    default:
    case NotLoadedIcon: {
        static const QIcon notLoaded
            = QIcon(":/pluginengine/images/notloaded.png");
        return notLoaded;
    }
    }
}
    ...
    QWidget(parent)
{
    ...
    m_model->setHeader(
        QStringList() << tr("Name") << tr("Load")
                      << tr("Version") << tr("Vendor"));
...
void PluginView::updatePlugins()
{
    ...
    Utils::sort(collections, [](CollectionItem *a, CollectionItem *b) -> bool { return a->m_name < b->m_name; });
