...
/*!
    \class PluginEngine::PluginDependency
    ...
    See also PluginEngine::IPlugin for more information about plugin dependencies and
    version matching.
*/

/*!
    \variable PluginEngine::PluginDependency::name
    String identifier of the plugin.
*/

/*!
    \variable PluginEngine::PluginDependency::version
    Version string that a plugin must match to fill this dependency.
*/

/*!
    \variable PluginEngine::PluginDependency::type
    Defines whether the dependency is required or optional.
    \sa PluginEngine::PluginDependency::Type
*/

/*!
    \enum PluginEngine::PluginDependency::Type
...
/*!
    \enum PluginEngine::PluginSpec::State
...
uint PluginEngine::qHash(const PluginDependency &value)
{
...
/*!
    A QRegularExpression matching the platforms this plugin works on. An empty pattern implies all platforms.
    \since 3.0
*/
...
bool PluginSpec::isAvailableForHostPlatform() const
{
    QRegularExpressionMatch match
        = d->platformSpecification.match(PluginManager::platformName());
    return d->platformSpecification.pattern().isEmpty() || match.hasMatch();
}
...
/*!
    Returns false if the file does not represent a InoCobotTP plugin.
*/
...
{
    return QCoreApplication::translate("PluginSpec", "The key \"%1\" is not a string.")
        .arg(QLatin1String(key));
}

static inline QString msgValueIsNotABool(const char *key)
{
    return QCoreApplication::translate("PluginSpec", "The key \"%1\" is not a bool value.")
        .arg(QLatin1String(key));
}

static inline QString msgValueIsNotAObjectArray(const char *key)
{
    return QCoreApplication::translate("PluginSpec", "The key \"%1\" is not an array of objects.")
        .arg(QLatin1String(key));
}

static inline QString msgValueIsNotAMultilineString(const char *key)
{
    return QCoreApplication::translate("PluginSpec", "The key \"%1\" is not a string  and not an array of strings.")
        .arg(QLatin1String(key));
}

static inline QString msgInvalidFormat(const char *key, const QString &content)
{
    return QCoreApplication::translate("PluginSpec", "The value %2 of the key \"%1\" is in an illegal format.")
...
bool PluginSpecPrivate::readMetaData(const QJsonObject &metaData)
{
    ...
    if (!value.isObject())
        return reportError(tr("Plugin data not found."));
        ...
        foreach (const QJsonValue &v, array) {
            ...
            if (!value.isUndefined()) {
                ...
                if (typeValue.toLower() == QLatin1String(DEPENDENCY_TYPE_HARD)) {
                    ...
                    return reportError(tr("Dependency: \"%1\" must be \"%2\" or \"%3\" or \"%4\"")
...
QRegularExpression &PluginSpecPrivate::versionRegExp()
{
    static QRegularExpression reg(QLatin1String("([0-9]+)(?:[.]([0-9]+))?(?:[.]([0-9]+))?(?:_([0-9]+))?"));
    return reg;
}
...
{
    QRegularExpression reg1 = versionRegExp();
    QRegularExpression reg2 = versionRegExp();

    QRegularExpressionMatch match1 = reg1.match(version1);
    bool isExactMatch1
        = match1.hasMatch()
       && (match1.capturedStart() == 0)
       && (match1.capturedLength() == version1.length());
    if (!isExactMatch1)
        return 0;

    QRegularExpressionMatch match2 = reg1.match(version2);
    bool isExactMatch2
        = match2.hasMatch()
       && (match2.capturedStart() == 0)
       && (match2.capturedLength() == version2.length());
    if (!isExactMatch2)
        return 0;
    int number1;
    int number2;
...
bool PluginSpecPrivate::resolveDependencies(const QList<PluginSpec *> &specs)
{
    ...
    if (state != PluginSpec::Read) {
        errorString = QCoreApplication::translate("PluginSpec", "Failed to read dependencies as the statut is not Read.");
        hasError = true;
        return false;
    }
    ...
    foreach (const PluginDependency &dependency, dependencies) {
        ...
        if (!found) {
            if (dependency.type == PluginDependency::Required) {
                ...
                errorString.append(QCoreApplication::translate("PluginSpec", "Failed to resolve dependency '%1(%2)'")
                                       .arg(dependency.name)
    ...
    dependencySpecs = std::move(resolvedDependencies);
...
bool PluginSpecPrivate::loadLibrary()
{
    ...
    if (state != PluginSpec::Resolved) {
        if (state == PluginSpec::Loaded)
            return true;
        errorString = QCoreApplication::translate("PluginSpec", "Failed to load plugin as the plugin status is not Resolved.");
        hasError = true;
        return false;
    }
    if (!loader.load()) {
    ...
    if (!pluginObject) {
        hasError = true;
        errorString = QCoreApplication::translate("PluginSpec", "Plugin is not valid because the parent class of the plugin is not IPlugin.");
        loader.unload();
        return false;
    }
    pluginObject->setObjectName(q->name());
    PluginManager::addObject(pluginObject);
...
bool PluginSpecPrivate::initializePlugin()
{
    ...
    if (state != PluginSpec::Loaded) {
        if (state == PluginSpec::Initialized)
            return true;
        errorString = QCoreApplication::translate("PluginSpec", "Failed to initialize plugin as the plugin is not loaded.");
        hasError = true;
        return false;
    }
    if (!plugin) {
        errorString = QCoreApplication::translate("PluginSpec", "Internal error: No plugin instance to initialize");
        hasError = true;
        return false;
    }
    QString err;
    ...
    if (currentDir.cd(QString("%1/plugins").arg(RELATIVE_SHARE_PATH)))
        if (!currentDir.cd(plugin->d->pluginSpec->name()))
            currentDir.mkdir(plugin->d->pluginSpec->name());
...
bool PluginSpecPrivate::initializeExtensions()
{
    ...
    if (state != PluginSpec::Initialized) {
        if (state == PluginSpec::Running)
            return true;
        errorString = QCoreApplication::translate("PluginSpec", "Unable to perform extension initialization, plugin status is not initialized.");
        hasError = true;
        return false;
    }
    if (!plugin) {
        errorString = QCoreApplication::translate("PluginSpec", "Internal error: No plugin instance capable of expansion to perform initialisation of expansion.");
        hasError = true;
        return false;
    }
...
bool PluginSpecPrivate::delayedInitialize()
{
    ...
    if (state != PluginSpec::Running)
        return false;
    if (!plugin) {
        errorString = QCoreApplication::translate("PluginSpec", "Internal error: No plugin instance for delayed initialization");
        hasError = true;
        return false;
    }
