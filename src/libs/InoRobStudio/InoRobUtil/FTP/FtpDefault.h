...
/**
 * @brief FTP二次封装类
 * 注意！！！此类因ftp的特性，使用时需当作局部变量调用，这样出了作用域才会在析构的时候进行断开ftp连接，否则将引起ftp异常
 */
class INOROBUTIL_CLASS FtpDefault : IFtp
{
public:
    FtpDefault(const std::string &ip);
    ~FtpDefault();
...
private:
     ...
     * @return
     */
    int mkParentDir(const std::string &rootPath, const std::string &filePath);

private:
    ftp::client _client;   // ftp客户端(析构时会自动断开ftp连接)
    std::string _ip = "";  // ip地址
};
