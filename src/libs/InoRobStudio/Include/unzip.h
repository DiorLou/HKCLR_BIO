...
DLL_ZIPAPI HZIP OpenZip(const TCHAR *fn, const char *password);
DLL_ZIPAPI HZIP OpenZip(void *z, unsigned int len, const char *password);
DLL_ZIPAPI HZIP OpenZipHandle(HANDLE h, const char *password);
...
DLL_ZIPAPI ZRESULT GetZipItem(HZIP hz, int index, ZIPENTRY *ze);
...
DLL_ZIPAPI ZRESULT FindZipItem(HZIP hz, const TCHAR *name, bool ic, int *index, ZIPENTRY *ze);
// FindZipItem - finds an item by name. ic means 'insensitive to case'.
// It returns the index of the item, and returns information about it.
// If nothing was found, then index is set to -1 and the function returns
// an error code.

DLL_ZIPAPI ZRESULT UnzipItem(HZIP hz, int index, const TCHAR *fn);
DLL_ZIPAPI ZRESULT UnzipItem(HZIP hz, int index, void *z, unsigned int len);
DLL_ZIPAPI ZRESULT UnzipItemHandle(HZIP hz, int index, HANDLE h);
...
DLL_ZIPAPI ZRESULT SetUnzipBaseDir(HZIP hz, const TCHAR *dir);
...
DLL_ZIPAPI ZRESULT CloseZipU(HZIP hz);
DLL_ZIPAPI unsigned int FormatZipMessageU(ZRESULT code, TCHAR *buf, unsigned int len);
DLL_ZIPAPI bool IsZipHandleU(HZIP hz);
#ifdef _zip_H
    #undef CloseZip
