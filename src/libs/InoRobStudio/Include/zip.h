...
#ifdef ZIP_EXPORTS
    #define DLL_ZIPAPI __declspec(dllexport)
#else
    #define DLL_ZIPAPI __declspec(dllimport)
#endif
...
DLL_ZIPAPI HZIP CreateZip(const TCHAR *fn, const char *password);
DLL_ZIPAPI HZIP CreateZip(void *buf, unsigned int len, const char *password);
DLL_ZIPAPI HZIP CreateZipHandle(HANDLE h, const char *password);
...
DLL_ZIPAPI ZRESULT ZipAdd(HZIP hz, const TCHAR *dstzn, const TCHAR *fn);
DLL_ZIPAPI ZRESULT ZipAdd(HZIP hz, const TCHAR *dstzn, void *src, unsigned int len);
DLL_ZIPAPI ZRESULT ZipAddHandle(HZIP hz, const TCHAR *dstzn, HANDLE h);
DLL_ZIPAPI ZRESULT ZipAddHandle(HZIP hz, const TCHAR *dstzn, HANDLE h, unsigned int len);
DLL_ZIPAPI ZRESULT ZipAddFolder(HZIP hz, const TCHAR *dstzn);
...
DLL_ZIPAPI ZRESULT ZipGetMemory(HZIP hz, void **buf, unsigned long *len);
...
DLL_ZIPAPI ZRESULT CloseZip(HZIP hz);
// CloseZip - the zip handle must be closed with this function.

DLL_ZIPAPI unsigned int FormatZipMessage(ZRESULT code, TCHAR *buf, unsigned int len);
...
DLL_ZIPAPI ZRESULT CloseZipZ(HZIP hz);
DLL_ZIPAPI unsigned int FormatZipMessageZ(ZRESULT code, char *buf, unsigned int len);
DLL_ZIPAPI bool IsZipHandleZ(HZIP hz);
#ifdef _unzip_H
    #undef CloseZip
