...
class QSetInsertIterator : public std::iterator<std::output_iterator_tag, void, void, void, void>
{
protected:
    Container *container;
...
struct TransformImpl {
    template <typename F>
    Q_REQUIRED_RESULT static C call(const SC &container, F function)
    {
        C result;
    ...
    Q_REQUIRED_RESULT static C call(const SC &container, R (S::*p)() const)
    {
...
template <typename C,  // container
          typename F>
Q_REQUIRED_RESULT auto transform(const C &container, F function)
    -> typename ContainerType<C>::template ResultOfTransform<F>::type
{
    return TransformImpl<
        typename ContainerType<C>::template ResultOfTransform<F>::type,
        C>::call(container, function);
}
...
template <typename C,
          typename R,
          typename S>
Q_REQUIRED_RESULT auto transform(const C &container, R (S::*p)() const)
...
Q_REQUIRED_RESULT auto transform(const SC &container, F function)
...
template <template <typename> class C,  // result container type
          ...
          typename R,
          typename S>
Q_REQUIRED_RESULT auto transform(const SC &container, R (S::*p)() const)
    -> C<decay_t<R>>
{
    return TransformImpl<
        C<decay_t<R>>,
...
Q_REQUIRED_RESULT
    C
    filtered(const C &container, F predicate)
{
    C out;
...
Q_REQUIRED_RESULT
    C
    filtered(const C &container, R (S::*predicate)() const)
{
    C out;
    std::copy_if(container.begin(), container.end(),
...
Q_REQUIRED_RESULT
    std::tuple<C, C>
    partition(const C &container, F predicate)
{
    C hit;
    C miss;
...
Q_REQUIRED_RESULT
    std::tuple<C, C>
    partition(const C &container, R (S::*predicate)() const)
{
...
Q_REQUIRED_RESULT
    C
    filteredUnique(const C &container)
{
    C result;
