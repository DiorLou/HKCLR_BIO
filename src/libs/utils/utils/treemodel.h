...
public:
    ...
    virtual bool preVisit(TreeItem *)
    {
        return true;
    }
...
public:
    ...
    explicit TreeItem(const QStringList &displays, const QString &icon,
                      int flags = Qt::ItemIsEnabled);
    virtual ~TreeItem();
...
private:
    ...
    void clear();
    bool removeOneChildren(int pos);
...
// Enable QMetaObject::invokeMethod return values using Utils::TreeItem*.
...
public:
    ...
    public:
        ...
        TreeItem *operator*()
        {
            return m_item[m_depth];
        }

        void operator++()
        {
            INO_ASSERT(m_depth == m_level, return );
...
public:
    ...
    explicit TreeLevelItems(const UntypedTreeLevelItems &items) :
        m_items(items)
    {
    }
    ...
    struct const_iterator : public UntypedTreeLevelItems::const_iterator {
        ...
        const_iterator(const UntypedTreeLevelItems::const_iterator &it) :
            UntypedTreeLevelItems::const_iterator(it)
        {
        }
        T operator*()
        {
            return static_cast<T>(UntypedTreeLevelItems::const_iterator::operator*());
        }
    };
...
public:
    ...
    QModelIndex indexForItem(const TreeItem *needle, const int col = 0) const;
    ...
    void beginResetModelCommandFromExtern();
    void endResetModelCommandFromExtern();
