...
class Communication;
class AbstractCmd;
class RobotPointInterface;

namespace Internal {
class COMMUNICATION_EXPORT RobotPointInterfacePrivate
{
public:
    RobotPointInterfacePrivate(RobotPointInterface *interface);
   ...
   // current loaded robot point file
    ...
    void setAbsoluteZeroPoint(AbstractCmd *);
    bool getAbsoluteZeroPoint(QList<double> &pos,
                              QList<int> &min,
                              QList<int> &max);
    void getAbsoluteZeroCurrentValue(AbstractCmd *);
    ...
    void setWorikOriginPointTriggerData(AbstractCmd *);
    void getWorikOriginData(AbstractCmd *);
...
private:
    // read current point
    bool readCurPoint(InoRobPos &point);

private:
    RobotPointInterface *q;
    std::string m_sRPointFileName = DEFAULT_ROBOT_POINT_FILE.toStdString();
    std::string m_sLoadedRPointFileName = DEFAULT_ROBOT_POINT_FILE.toStdString();
};
}
