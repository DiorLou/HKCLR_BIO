...
class Communication;
class AbstractCmd;
class ControllerSystemConfig;

namespace  Internal{
class COMMUNICATION_EXPORT ControllerSystemConfigTr:public QObject
{
    Q_OBJECT
};

class COMMUNICATION_EXPORT ControllerSystemConfigPrivate
{
public:
    ControllerSystemConfigPrivate(ControllerSystemConfig *config);
...
private:
    ControllerSystemConfig *q;
    void startConfigFileOperation(int funCode, AbstractCmd *cmd);
    void readConfigFileToUsbOperationState(AbstractCmd *cmd);
    QString getErrorStr(int code);

    bool m_hasDownLoadConfigFile = false;
    bool m_hasfinishLastImport = false;
    QDateTime timeForExportFile;
    QDateTime timeForImportFile;
    bool m_lastControllerBackUpFileWithUsbHasFinish = false;
};
}
